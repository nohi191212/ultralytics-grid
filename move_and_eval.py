...
def write_evaluation_result(result_dir="/mnt/workspace/inference", 
                           result_file="statistic.jsonl",
                           data_dict=None):
    """
    写入评估结果到JSONL文件
...
def main():
    ...
    parser.add_argument('--result-dir', '-r',
                       default='/mnt/workspace/inference',
                       help='结果存放目录 (默认: /mnt/workspace/inference)')
    parser.add_argument('--result-file', '-f',
                       default='statistic.jsonl',
                       help='结果文件名 (默认: statistic.jsonl)')
    parser.add_argument('--ap50', '-a',
                       type=float,
                       default=0.6568,
                       help='AP50分数 (默认: 0.6568)')
    parser.add_argument('--dataset-name', '-d',
                       default='Omni问财打点测试集v1_1-0-0',
                       help='数据集名称')
    ...
    try:
        ...
        print("=" * 60)
        final_path = move_file_to_checkpoint(args.source, args.checkpoint_dir)
